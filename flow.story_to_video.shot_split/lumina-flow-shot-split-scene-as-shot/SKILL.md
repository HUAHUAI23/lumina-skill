...
**「场景」= 1 个 shot = 1 次 Seedance 调用 = 用户口中的「1 个视频」**，是镜头之上的聚合单位。

- 每个场景声明时长 **默认 12-15s**（贴 Seedance 一次生成硬上限 15s 走，最大化单次产出）。
- **用户没有显式指定时长时按默认 12-15s 校验**；偏离时 flag「场景 X 时长 Y.Ys 偏离默认 12-15s，确认是否用户显式指定了其他时长」。
- **用户显式指定时长**（在用户提示里写如「一个视频建议 8-10s」/「每段 6 秒」/「每场景不超过 10s」/「每场景 12s」）时，按用户值校验，不再用 12-15s flag。
- Seedance 单次生成硬上限 ≤15s：任何场景声明 > 15s 时 flag「场景 X 时长 Y.Ys 超过 Seedance 一次生成上限 15s」，仍按原文落输出（shot.duration 用镜头之和），**不擅自截断**。
...
scene-as-shot 不改写原镜头行描述，但要在质量说明里 flag 触发 Seedance 2.0 "人物闪现 / 拍间融合" 反模式的写法。检查每个 block.description 首句：

- **环境前置**：首句以"明亮的 X / 阳光 / 冷光 / 暖光 / 整间 / 一片"等环境短语开头，接"镜头推近到 X / 镜头摇过到 X"，flag「场景 X 镜头 N 首句环境前置，Seedance 2.0 易出现人物闪现，建议回 consult.storyboard-script-to-shot-prompts 改为主体前置（先写主体位置 + 动作，再写环境）」。
- **被动揭示语**：出现"揭示出 X / 最终落在 X 身上 / 原来 X 站在 / 突然出现 X / 忽然闪现 X / 画面里多了 X"，flag「场景 X 镜头 N 出现被动揭示语，主体可能延迟揭示」。
- **carry-over 角色缺位**：上一镜（同场景或前一场景末镜）的同一角色在本镜首句没有显式出现（无位置/状态描述），flag「场景 X 镜头 N carry-over 角色 X 未在首句重提位置/状态」。
- **新进角色无入画方向**：本镜首次出现的角色没写方向/起始位置（"从画面左侧入画"），flag「场景 X 镜头 N 新进角色 X 缺入画方向」。

scene-as-shot **不擅自改写** block.description，按原文落输出；只在质量说明里 flag，让用户决定回 consult 层修。

## 焦距 jargon flag

`[景别/运镜]` 字段或 block.description 里出现 `85mm / f/1.4 / 35mm 焦段 / shallow DOF / focal plane / 浅景深 / 焦距 / 光圈 / 景深` 等技术 jargon 时 flag「场景 X 镜头 N 出现焦距 jargon，Seedance 不响应甚至 degrade，建议改 `广角空间感 / 自然视角 / 人像压缩感 / 微距质感 + 画面占比`」。
...
- 每个 shot.duration 落在 **默认 12-15s**（用户没指定时）或 **用户显式指定值**（如「一个视频 8-10s」）范围内；偏离已 flag。用户没指定就一律走默认，不擅自改判范围。
...
- 场景声明时长偏离默认 12-15s **且用户没有显式指定其他时长**时（用户没指定就一律走默认），flag「场景 X 时长 Y.Ys 偏离默认 12-15s，可能是上游格式漂移或用户未声明的特殊时长需求」，仍按原文输出。
- 用户在 prompt 里显式指定了其他时长（如「一个视频 8-10s」）时，按用户值校验，不再用 12-15s 这条 flag。
