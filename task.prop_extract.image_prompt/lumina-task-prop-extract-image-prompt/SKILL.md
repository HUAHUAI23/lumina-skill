...
- 默认输出 1 张基准单体图；只有解析事实显示同一道具需要额外角度或材质细节锚点时，才写入 variants。
- 如果写入 variants，第一张必须是 `canonical_object`：基准文生图，单体居中、完整可见，清楚展示轮廓、主要材质和稳定标记。
- 可按事实需要补充 `three_quarter_object`：后续图生图变体，保持同一物体身份，用轻微透视或三分之四角度补足体积和可运动参考。
