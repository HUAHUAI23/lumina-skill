...
description: router.consult_only 的补充提示词；用于未显式指定链路时在 consult 路径中选路
---
...
- 从当前消息里识别主要咨询意图。
- 在 `consult.general`、`consult.narrative`、`consult.screenplay`、`consult.image_prompt`、`consult.video_prompt`、`consult.storyboard` 中选最合适的一条。
...
- 用户要把叙事、梗概、小说段落或故事想法整理成可确认剧本文本，走 `consult.screenplay`。
