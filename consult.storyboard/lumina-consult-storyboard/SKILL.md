...
- "动作与调度"栏可以按节拍组织：`m1: 全景缓慢推向冷蓝门口空镜`，`m2: 中近景固定，小美停在门外看向玻璃后人影`。
- "视线与空间"栏针对高风险位移用 SDV 句式：`<起点锚点> → <X+/X-/Y+/Y-/Z+/Z-> → <终点锚点>`。
- "台词/旁白"明确挂在哪个 micro-shot；同一句台词不重复挂多拍；offscreen 挂听者反应拍。
- "剪辑承接"栏写清切点落在哪个 micro-shot 的尾帧，让下游剪辑师知道该镜头的"出门"在哪里。
...
- 原子化：每个 ≥4s 镜头的"动作与调度"是否能自然拆成节拍；每拍是否有清楚镜头重心。
- 节拍：关键转折、揭示和余震是否用自然短语标注；重音拍是否清楚。
- 台词：是否避免直白解释；关键台词是否被反应承接；语速是否可听；台词是否挂在能承载口型/反应的 micro-shot。
